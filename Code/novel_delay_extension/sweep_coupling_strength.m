%% Sweep coupling strength kappa to find a non-degenerate operating regime
%
% Stage 7's first attempt (kappa=6, "hand-tuned, not fit to data" per the
% recovered script's own comment) reached near-total synchronization
% (R~0.98) within the simulation window, leaving cos(theta(t)) nearly
% rank-1 (95%+ variance in a single SVD mode) -- a degenerate basis for
% reconstruction purposes. This sweeps kappa at the same reference speed
% (5 m/s) to find a regime with sustained spatial complexity instead,
% using the participation ratio (effective number of contributing modes,
% (sum(sv^2))^2 / sum(sv^4)) as a direct measure of basis richness,
% alongside the order parameter R for context.
...
%% Rebuild parcel-level EDR-only / EDR+LR graphs (same as before)
...
kappas = [0.1, 0.25, 0.5, 1, 2, 3, 4, 6];
n_k = numel(kappas);
R_edr = nan(1,n_k); R_edrlr = nan(1,n_k);
peff_edr = nan(1,n_k); peff_edrlr = nan(1,n_k);

fprintf('%8s | %10s %10s | %12s %12s\n', 'kappa', 'R (EDR)', 'R (EDR+LR)', 'P_eff (EDR)', 'P_eff (EDR+LR)');
for ki = 1:n_k
    kappa = kappas(ki);
    ...
    R_edr(ki) = mean(R_e); R_edrlr(ki) = mean(R_el);

    X_edr = cos(theta_edr(n_transient_steps+1:end, :))';
    X_edrlr = cos(theta_edrlr(n_transient_steps+1:end, :))';
    sv_edr = svd(X_edr, 'econ');
    sv_edrlr = svd(X_edrlr, 'econ');
    peff_edr(ki) = (sum(sv_edr.^2))^2 / sum(sv_edr.^4);
    peff_edrlr(ki) = (sum(sv_edrlr.^2))^2 / sum(sv_edrlr.^4);

    fprintf('%8.2f | %10.4f %10.4f | %12.2f %12.2f\n', kappa, R_edr(ki), R_edrlr(ki), peff_edr(ki), peff_edrlr(ki));
end

fg = figure('Name', 'Coupling strength sweep', 'Visible', 'off');
subplot(1,2,1)
plot(kappas, R_edr, 'o-', 'LineWidth', 1.5); hold on
plot(kappas, R_edrlr, 's-', 'LineWidth', 1.5); grid on
xlabel('Coupling strength \kappa'); ylabel('Order parameter R'); ylim([0 1])
legend('EDR', 'EDR+LR', 'Location', 'best'); title('Synchronization vs. coupling')
subplot(1,2,2)
plot(kappas, peff_edr, 'o-', 'LineWidth', 1.5); hold on
plot(kappas, peff_edrlr, 's-', 'LineWidth', 1.5); grid on
xlabel('Coupling strength \kappa'); ylabel('Effective # modes (participation ratio)')
legend('EDR', 'EDR+LR', 'Location', 'best'); title('Basis richness vs. coupling')
exportgraphics(fg, fullfile(fig_out_dir, 'coupling_sweep.png'));

save(fullfile(project_root, 'Results', 'novel_delay_extension', 'coupling_sweep.mat'), ...
    'kappas', 'R_edr', 'R_edrlr', 'peff_edr', 'peff_edrlr');
fprintf('\nSaved -> %s\n', fullfile(fig_out_dir, 'coupling_sweep.png'));
