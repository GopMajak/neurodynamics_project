...
%%%    a path-setup block that finds the Pang + Vohryzek toolboxes relative
%%%    to this file, and cd's into the vohryzek2024_EDRLR folder so the
%%%    original script's relative paths ('Data/...', 'Results/...',
%%%    'Connectome_derivation/...') resolve exactly as in the original repo.
%%%  - the single daviolinplot(...) call (a third-party FileExchange
%%%    dependency not present in either toolbox) is replaced with an
%%%    equivalent base-MATLAB boxplot+jittered-scatter. This is a plotting
%%%    substitution only -- no analysis/statistics code below this point
%%%    is changed.
%%%  All other logic is untouched from the original repo.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% repo_dir = '/BrainEigenmodes_EDRLR-main';   % ORIGINAL (machine-specific) -- superseded by PATCH above
...
figure
% --- PATCH: daviolinplot (third-party FileExchange, not available)
% replaced with equivalent base-MATLAB boxplot + jittered scatter.
% ORIGINAL:
...
d = data2mse(:,1);
g = group_inx(:);
boxplot(d, g, 'Labels', condition_names(1:4), 'Colors', c, 'Symbol', 'k+');
hold on
for gi = 1:4
    yi = d(g == gi);
    xi = gi + (rand(size(yi)) - 0.5) * 0.2;
    scatter(xi, yi, 10, c(gi, :), 'filled', 'MarkerFaceAlpha', 0.5);
end
% --- END PATCH
ylabel('MSE');
xl = xlim; xlim([xl(1)-0.1, xl(2)+0.2]); % make more space for the legend
set(gca,'FontSize',10); grid on
exportgraphics(gcf, fullfile(fig_out_dir, 'Figure2_MSE_at_200modes_boxplot.png')); % PATCH: save for headless run
...
set(gcf,'Color', [1 1 1]);
exportgraphics(gcf, fullfile(fig_out_dir, 'Figure2_MSE_vs_modes.png')); % PATCH: save for headless run
