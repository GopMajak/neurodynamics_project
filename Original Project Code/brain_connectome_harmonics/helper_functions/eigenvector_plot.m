function eigenvector_plot( x, p, cax, cm )
% Plots a value at each node's 3D position, colored by that value.
% Used for visualizing an eigenmode (or any per-node data) on the network.
%
% Inputs:
...
h = nan( N, 1 );
for ii = 1:N
	h(ii) = plot3( p(ii,1), p(ii,2), p(ii,3), 'k.', 'markersize', 90 );
end
